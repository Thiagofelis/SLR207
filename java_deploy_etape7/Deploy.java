...
public class Deploy
{
    ...
    FileWriter out = new FileWriter("valid_machines.txt");
		...
		while(line != null)
		{
			...
			{
				totalNumMachines++;
				if ((i+1) < 10) array[i] = new ConnexionVerifier("tp-" + room + "-0" + (i+1), out, validMachines, workingMachines);
				else						array[i] = new ConnexionVerifier("tp-" + room + "-" + (i+1), out, validMachines, workingMachines);
				array[i].start();
			}
			myList.add(array);
